...
def main():
    ...
    wandb.config.augment_timnet_padtowidth = 64
    ...
    #wandb.config.eval_splits = ["train[:10%]", "validation[:10%]"]
    ...
    wandb.config.enable_reinit = True
    wandb.config.enable_width_expansion = True
    wandb.config.enable_depth_expansion = True
    wandb.config.depth_score_max_k = 8
    ...
    final_checkpoint_dir = f"/senn/orbax/{run_id}/final"
    ...
    def final_save(state):
        ...
        checkpointer.save(final_checkpoint_dir, state, save_args=ckpt_save_args)
