...
def main():
    cfg = experiment_utils.get_cfg('experiment3')
    ...
    def get_evaluator(layer_index, size=None):
        ...
        return jax.experimental.maps.xmap(
            lambda state, ngrad, grad, feature, pair, eps: kfac_direct_sgd(
                solver.model,
    ...
    def consider_adding_width(evaluator, validator, layer_index):
        ...
        if vmetric/vlayer_score > cfg['evo']['thresh'].get() and best_raw/loss_sqnorm > cfg['evo']['abs_thresh'].get():
            new_state = proposer.embed_feature(solver.state, get_feature(best), layer_index)
    ...
    def consider_inserting_layer(evaluator, validator, layer_index):
        ...
        adjusted_vmetric = vmetric / cfg['evo']['layer_cost_mul'].get()
        print(f"layer {layer_index} adjusted vratio: {adjusted_vmetric/vlayer_score:.5f}")

        if adjusted_vmetric/vlayer_score > cfg['evo']['thresh'].get():# and adjusted_vmetric/loss_sqnorm > cfg['evo']['abs_thresh'].get():
            ...
            # exit()
            adjusted_ratio = 1. + adjusted_vmetric/baseline
