...
class SklGenClass(Classification):
    ...
    def get_data(self, _, test=False, index=0):
        ...
        assert data_cfg['name'].get(None) == 'make_moons', "only make_moons is supported"
        from sklearn.datasets import make_moons
        data, labels = make_moons(
            n_samples = data_cfg['TN'].get() if test else data_cfg['N'].get(),
            noise = data_cfg['noise'].get(),
            random_state = data_cfg['test_seed'].get() if test else data_cfg['train_seed'].get()
        )
...
def kfac_mala_burst(fmodel, act, Ginv, res, feature, key, lr, temp=1e0, atikh=0., steps=10, score_norm=1e0):
    ...
    feature, accept_rate = mala_steps(loss_grad, priorvar, feature, key, lr, steps, temp=temp)
...
def main():
    ...
    def consider_adding_width(evaluator, validator, layer_index, final=False):
        ...
        landscape_results.update(dict(neuron_positions=neuron_positions, neuron_scales=neuron_scales))
        ...
        landscape_results.update(dict(init_positions=init_positions, init_scales=init_scales))
        ...
        vis_x0 = jnp.linspace(-3., 3., 100)
        vis_x1 = jnp.linspace(-3., 3., 100)
        ...
        for i in range(1):
            ...
            best = jax.random.categorical(next(proposer.key_iter), logits=opt_state.rmetrics/(temp*layer_score), shape=())
            ...
            best_raw = jnp.sum(jax.nn.softmax(opt_state.rmetrics/(temp*layer_score))*opt_state.rmetrics)
        ...
        landscape_results.update(dict(final_positions=final_positions, final_scales=final_scales))
        ...
        print(f"layer {layer_index} best_ratio: {best_raw/layer_score:.8f} ({best_raw:.8f}/{layer_score:.8f})")
        ...
        going_to_add = best_local_ratio > cfg['evo']['thresh'].get() and best_raw/loss_sqnorm > cfg['evo']['abs_thresh'].get()
    ...
    def consider_inserting_layer(evaluator, validator, layer_index):
        ...
        # landscape_results.update(dict(x0=vis_x0, x1=vis_x1, pred=vis_y))
        # print("Saving feature landscape...")
        # np.savez(f"./results/landscape{9999:04d}", **landscape_results)
        # exit()
    ...
    for epoch in range(initial_epoch, max_epochs):
        ...
        if epoch % cfg['evo']['cooldown'].get(20) == 0 or is_final:
            ...
            def consider(stuff):
                ...
                if cont is not None:
                    if cont < cap - 1:
                        return consider_adding_width(evaluators[layeridx], validators[layeridx], layeridx, final=is_final)
                    else:
                        return None
                else:
