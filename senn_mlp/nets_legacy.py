...
class Passive(nn.Module):
    ...
    def restrict_params(self, dim):
        params = self.variables["params"]
        restrict = lambda arr: arr.at[..., dim:].set(0.)
...
class DDense(nn.Dense, Splittable):
    ...
    def restrict_params(self, dim):
        params = super().restrict_params(dim)
        # print(params)
        return params.copy({
            "kernel": params["kernel"].at[dim:, :].set(0.)
        })
...
class Layer(Splittable):
    ...
    def restrict_params(self, dim):
        params = self.variables["params"]
        params = params.copy({
            'linear': self.linear.restrict_params(dim)
        })
        params = params.copy({
            f'equivariant_{i}': eq.restrict_params(dim) \
            for i, eq in enumerate(self.equivariant) \
...
class Layers(nn.Module):
    ...
    def restrict_params(self, dims):
        ...
        def modify(layer, dim):
            if dim is None:
                return layer.identity_params()
            else:
                return layer.restrict_params(dim)
        ...
        return state.copy({
            "params": params.copy({
                get_key(i): modify(layer, dim) \
                    for i, (layer, dim) in enumerate(zip(self.layers, dims))
            })
        })
